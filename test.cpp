...
{   
	...
	Conj<int> ifaces1;
	ifaces1.Agregar(1);
	Compu c1("Compu:c1", ifaces1);
	
	Conj<int> ifaces2;
	ifaces2.Agregar(1);
	ifaces2.Agregar(2);
	Compu c2("Compu:c2", ifaces2);
	
	Conj<int> ifaces3;
	ifaces3.Agregar(3);
	Compu c3("Compu:c3", ifaces3);
	
	::Paquete p(555, 77, c1, c2);
	::Paquete q(556, 57, c1, c3);
	::Paquete r(557, 42, c2, c1);
	...
	rr.Conectar(c1,c2,1,1);
	
	rr.Conectar(c2,c3,2,3);
	
	cout << "aca" << rr.Interfaces(3)<< endl; 
	DcNet dc(&rr);
	...
	cout <<"1 " <<dc.enEspera(c1)<< endl;
	cout <<"2 " <<dc.enEspera(c2)<< endl;
	cout <<"3 " <<dc.enEspera(c3)<< endl;
	...
	cout <<"camino recorridfo"  << q << " " << dc.caminoRecorrido(q)<<endl;	
	cout <<"1 " <<dc.enEspera(c1)<< endl;
	cout <<"2 " <<dc.enEspera(c2)<< endl;
	cout <<"3 " <<dc.enEspera(c3)<< endl;
...
*/
	...
	//~ dcnet.CrearPaquete(c1,c2,3);
	//~ cout << "enEspera" << 
	//~ cout << "cardinal " << dcnet.CantidadEnEsperaEn(c1) << endl;
	//~ 
	//~ cout << "claves" << miConj.claves() << endl;
	//~ conjPaquete::iterador miIter = miConj.crearIt();
	//~ 

	//~ while(miIter.haySiguiente()) {
		//~ cout << "haySiguiente devuelve " << miIter.haySiguiente() << endl;
		//~ ::Paquete paux = miIter.siguiente();
					//~ cout << "siguiente devuelve " << paux << endl;
		//~ cout << "voy a avanzar..." << endl << endl;
		//~ miIter.avanzar();
	//~ 
	//~ }
	//~ 
	//~ cout << "claves2 " << miConj.claves() << endl;
	//~ conjPaquete::iterador miItera = miConj.crearIt();
	//~ 
	//~ 
	//~ while(miItera.haySiguiente()) {
		//~ cout << "haySiguiente devuelve2 " << miItera.haySiguiente() << endl;
		//~ ::Paquete pauxa = miItera.siguiente();
					//~ cout << "siguiente devuelve2 " << pauxa << endl;
		//~ cout << "voy a avanzar...2 " << endl << endl;
		//~ miItera.avanzar();
	//~ 
	//~ }
	//~ 
